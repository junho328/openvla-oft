"""
Observation history management utilities for Dual-Arm ACPPO VLA training.
...
class DualArmObservationHistoryManager:
    ...
    def _initialize_history(self):
        ...
        zero_proprio = np.zeros(DUALARM_ACPPO_CONSTANTS["MODEL_PROPRIO_DIM"], dtype=np.float32)
    ...
    def get_front_view_only_observation(
        ...
        """
        Get front view (agentview) only observation with wrist images padded to zeros.
        
        Used for estimating agent 0's action distribution for agent 1.
        
        Returns:
            Dictionary with:
            - 'images': List of [agentview_t, zero_pad_left, zero_pad_right, 
                               agentview_{t-1}, zero_pad_left, zero_pad_right, ...]
        """
        zero_image = np.zeros((*self.image_size, 3), dtype=np.uint8)
        ...
        if include_history and self.history_length > 1:
            ...
            for t in range(len(agentview_list) - 1, -1, -1):
                ...
                images.append(zero_image.copy())  # Pad left wrist with zeros
                images.append(zero_image.copy())  # Pad right wrist with zeros
            
            return {'images': images}
        else:
            return {
                'images': [
                    list(self.agentview_history)[-1],
                    zero_image.copy(),  # Pad left wrist
                    zero_image.copy(),  # Pad right wrist
                ],
            }
    ...
    def get_agent_observation(
        ...
        agent_idx: int,
        include_history: bool = True,
        action_dist_from_agent0: Optional[np.ndarray] = None,
    ) -> Dict[str, np.ndarray]:
        """
        ...
        - Agent 1: [zeros(7), proprio[7:]] -> 14-dim, optionally concatenated with action_dist
        
        Args:
            agent_idx: Agent index (0 or 1)
            include_history: Whether to include historical frames
            action_dist_from_agent0: For agent 1, estimated action distribution from agent 0
                                     Shape: (action_dist_dim,) where action_dist_dim = 7 * chunk * 2
            
        Returns:
            Dictionary containing:
            - 'images': List of images [agentview_t, left_wrist_t, right_wrist_t, 
                                       agentview_{t-1}, left_wrist_{t-1}, right_wrist_{t-1}, ...]
            - 'proprio': Current padded proprio state (14-dim for agent 0, 14+action_dist_dim for agent 1)
            - 'proprio_history': Historical padded proprio states (if include_history)
        """
        zero_image = np.zeros((*self.image_size, 3), dtype=np.uint8)
        agent_proprio_dim = DUALARM_ACPPO_CONSTANTS["AGENT_PROPRIO_DIM"]
        model_proprio_dim = DUALARM_ACPPO_CONSTANTS["MODEL_PROPRIO_DIM"]
        ...
        if include_history and self.history_length > 1:
            ...
            for t in range(len(proprio_list) - 1, -1, -1):
                ...
                if agent_idx == 0:
                    ...
                    # For agent 1, optionally concatenate action distribution from agent 0
                    if action_dist_from_agent0 is not None:
                        padded_proprio = np.concatenate([padded_proprio, action_dist_from_agent0])
    ...
    def get_all_agent_observations(
        ...
        """
        return [
            self.get_agent_observation(
                agent_idx, 
                include_history,
                action_dist_from_agent0 if agent_idx == 1 else None,
            )
...
def extract_bimanual_proprio_from_env(obs: Dict) -> np.ndarray:
    ...
    """
    agent_proprio_dim = DUALARM_ACPPO_CONSTANTS["AGENT_PROPRIO_DIM"]  # 7
...
def prepare_vla_input(
    ...
    Args:
        ...
        proprio: Padded proprio state for the agent (14-dim or 14+action_dist_dim)
...
def prepare_vla_input_for_action_dist_estimation(
    ...
    - Uses front view (agentview) images only
    - Wrist images are padded with zeros
    - Text instruction is for robot 0 (not robot 1)
    
    Args:
        front_images: List of front view (agentview) images (with history)
    ...
    # [agentview_t, zero_left, zero_right, agentview_{t-1}, zero_left, zero_right, ...]
    images_with_padding = []
    for front_img in front_images:
        images_with_padding.append(front_img)
        images_with_padding.append(zero_image.copy())  # Pad left wrist
        images_with_padding.append(zero_image.copy())  # Pad right wrist
    ...
    # Build prompt for Agent 0's action estimation
    prompt = f"In: You are robot 0. You need to cooperate with the other robot to complete {task_description.lower()} task. What action should you take to {task_description.lower()}?\nOut:"
    
    # Process primary image with text (same pattern as prepare_vla_input)
    inputs = processor(prompt, processed_images[0]).to(device, dtype=torch.bfloat16)
...
def unnormalize_action(
    ...
    agent_action_dim = DUALARM_ACPPO_CONSTANTS["AGENT_ACTION_DIM"]  # 7
