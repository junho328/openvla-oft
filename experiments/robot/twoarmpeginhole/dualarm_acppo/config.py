"""
Configuration and constants for Dual-Arm ACPPO training on TwoArmPegInHole environment.
...
ACPPO (Agent-Chained PPO) extends MAPPO by:
1. Agent 1 receives estimated action distribution from Agent 0
...
# Dual-Arm ACPPO specific constants for bimanual model
...
DUALARM_ACPPO_CONSTANTS = {
    ...
    # ACPPO: Action distribution dimensions for chaining
    # 7 action * num_chunk * 2 (mu + sigma) = 14 * num_chunk for each agent
    "ACTION_DIST_DIM_PER_AGENT": 14,          # 7 * 1 * 2 = 14 (mu + sigma) per chunk step
}


@dataclass
class DualArmACPPOConfig:
    """Configuration for Dual-Arm ACPPO training with bimanual model."""
    ...
    num_actions_chunk: int = 25                       # Action chunk size for open-loop
    ...
    # For bimanual: 7 action_dim * num_actions_chunk * 2 (mu + sigma)
    action_dist_dim: int = 350                        # 7 * 25 * 2 = 350 (auto-adjusted in __post_init__)
    ...
    # Agent 0: proprio_dim = 14 (padded bimanual)
    # Agent 1: proprio_dim = 14 + action_dist_dim (padded proprio + estimated action dist)
    proprio_dim_agent0: int = 14                      # Padded bimanual proprio
    proprio_dim_agent1: int = 364                     # 14 + 350 (auto-adjusted in __post_init__)
    ...
    num_steps_per_rollout: int = 256                  # Steps per rollout before update (chunks)
    ...
    log_dir: str = "./experiments/logs/dualarm_acppo"
    video_dir: str = "./rollouts/dualarm_acppo"
    ...
    def __post_init__(self):
        ...
        # For bimanual: 7 action_dim * num_actions_chunk * 2 (mu + sigma)
        expected_action_dist_dim = self.agent_action_dim * self.num_actions_chunk * 2
        ...
        # Update agent 1's proprio dim (padded proprio + action dist)
        self.proprio_dim_agent1 = self.model_proprio_dim + self.action_dist_dim
...
def get_config_from_args(args=None):
    """Create DualArmACPPOConfig from command line arguments."""
    import draccus
    
    @draccus.wrap()
    def _create_config(cfg: DualArmACPPOConfig) -> DualArmACPPOConfig:
        return cfg
