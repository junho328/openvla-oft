...
class MAPPOTrainer:
    ...
    def __init__(self, cfg: MAPPOConfig):
        ...
        # Uses ACPO advantage decomposition with per-agent values
        self.buffer = MultiAgentRolloutBuffer(
            ...
            history_length=cfg.history_length,
            gamma=cfg.gamma,
            gamma_prime=cfg.gamma_prime,  # ACPO microstep discount
            gae_lambda=cfg.gae_lambda,
            lambda_prime=cfg.lambda_prime,  # ACPO microstep GAE lambda
            device=self.device,
            store_images=True,
        )
    ...
    def collect_rollouts(self) -> Dict[str, float]:
        ...
        for step in range(self.cfg.num_steps_per_rollout):
            with torch.no_grad():
                ...
                # Forward pass - get per-agent values for ACPO advantage decomposition
                ...
                # Store per-agent values for ACPO
            ...
            # Store per-agent values for ACPO advantage decomposition
            self.buffer.add(
                ...
                values=[np.expand_dims(v, 0) for v in per_agent_values],  # ACPO: per-agent values
        ...
        # Compute final per-agent values for ACPO advantage decomposition
        with torch.no_grad():
            agent_obs = self.obs_history.get_all_agent_observations(include_history=True)
            agent_inputs = []
            agent_proprios = []
            ...
            # ACPO: convert to per-agent numpy arrays
            last_values_np = [v.float().cpu().numpy() for v in last_values]
        
        # ACPO: compute per-agent returns and advantages
    ...
    def update(self) -> Dict[str, float]:
        ...
        # Per-agent stats tracking for ACPO
        ...
        for epoch in range(self.cfg.num_epochs):
            for batch in self.buffer.get(batch_size):
                # ACPO: Use per-agent advantages for proper credit assignment
                ...
                # ACPO: Compute per-agent policy and value losses
                all_policy_losses = []
                all_value_losses = []
                all_clip_fracs = []
                all_approx_kl = []
                ...
                for agent_idx in range(TWOARM_MAPPO_CONSTANTS["NUM_AGENTS"]):
                    ...
                    agent_advantages = batch_agent_advantages[agent_idx]  # ACPO: per-agent advantage
                    agent_returns = batch.agent_returns[agent_idx]  # ACPO: per-agent returns
                    agent_old_values = batch.agent_old_values[agent_idx]  # ACPO: per-agent old values
                    new_values = all_new_values[agent_idx]  # ACPO: per-agent new values
                    
                    log_ratio = new_log_probs - agent_old_log_probs
                    ratio = torch.exp(log_ratio)
                    
                    # ACPO: Use per-agent advantage for policy loss
                    ...
                    # ACPO: Compute per-agent value loss
                ...
                value_loss = torch.stack(all_value_losses).mean().float()  # ACPO: averaged per-agent value losses
        ...
        # ACPO: Per-agent stats for credit assignment analysis
        for agent_idx in range(TWOARM_MAPPO_CONSTANTS["NUM_AGENTS"]):
        ...
        # ACPO diagnostic stats: value/advantage distributions per agent
        for agent_idx in range(TWOARM_MAPPO_CONSTANTS["NUM_AGENTS"]):
        ...
        # ACPO: Value head difference diagnostic
        # If V[0] ≈ V[1], intermediate agents' TD residuals may be near zero
