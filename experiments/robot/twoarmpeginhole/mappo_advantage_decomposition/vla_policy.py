...
class MultiAgentVLAPolicy(nn.Module):
    ...
    ACPO Key Feature:
    - Even when sharing policy (action head), each agent has its own Value Head
    - This enables per-agent value functions V^(i) for ACPO advantage decomposition
    ...
    def __init__(
        ...
        print_rank0(f"  ACPO: Separate Value Heads per agent: True")
        print_rank0("="*60 + "\n")
        
        if self.share_policy:
            # Create single shared agent with Action Head (Value Head will be separate)
            ...
            # ACPO: Create separate Value Heads for each agent V^(0), V^(1), ..., V^(N-1)
            # Even though policy is shared, each agent needs its own value function
            ...
            print_rank0(f"\nACPO: Created {self.num_agents} separate Value Heads:")
            for i in range(self.num_agents):
                vh_params = sum(p.numel() for p in self.per_agent_value_heads[i].parameters())
                print_rank0(f"  V^({i}): {vh_params:,} parameters")
        else:
            ...
            # When not sharing policy, each agent already has its own value head
            self.per_agent_value_heads = None
    ...
    def get_values(
        ...
        for agent_idx, agent in enumerate(self.agents[:self.num_agents]):
            ...
            # ACPO: Use per-agent value head if available
            if self.share_policy and self.per_agent_value_heads is not None:
                # Get hidden states from shared agent
                ...
                # Use agent-specific value head
                value = self.per_agent_value_heads[agent_idx](text_hidden_states)
            else:
                # Use agent's own value head
    ...
    def get_actions_and_values(
        ...
        for agent_idx, agent in enumerate(self.agents[:self.num_agents]):
            ...
            # ACPO: Use per-agent value head if available
            if self.share_policy and self.per_agent_value_heads is not None:
                # Get action and shared hidden states
                action, log_prob, entropy, _ = agent.get_action_and_value(
                    inputs=inputs,
                    proprio=proprio,
                    use_proprio=self.cfg.use_proprio,
                    use_film=self.cfg.use_film,
                ...
                # Get hidden states for per-agent value head
                text_hidden_states, _ = agent.forward(
                    inputs=inputs,
                    proprio=proprio,
                    use_proprio=self.cfg.use_proprio,
    ...
    def evaluate_actions_and_values(
        ...
        for agent_idx, agent in enumerate(self.agents[:self.num_agents]):
            ...
            # ACPO: Use per-agent value head if available
            if self.share_policy and self.per_agent_value_heads is not None:
                # Get log_prob and entropy from shared agent
                log_prob, entropy, _ = agent.evaluate_actions_and_value(
                    inputs=inputs,
                    actions=actions,
                    proprio=proprio,
                    use_proprio=self.cfg.use_proprio,
    ...
    def get_trainable_parameters(self) -> List[nn.Parameter]:
        ...
        - Per-agent Value head parameters (ACPO)
        ...
        if self.share_policy:
            # Use agent's method to get trainable params (action head, log_std, etc.)
            params.extend(self.shared_agent.get_trainable_parameters())
            
            # ACPO: Add per-agent value head parameters
            if self.per_agent_value_heads is not None:
                for i, vh in enumerate(self.per_agent_value_heads):
