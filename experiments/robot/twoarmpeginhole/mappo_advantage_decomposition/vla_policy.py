...
class VLAAgent(nn.Module):
    """
    Single VLA agent wrapper for MAPPO with ACPO (Actor-Critic).
    ...
    - Multiple Value Heads for ACPO per-agent credit assignment
    - Uses PPO loss instead of L1 loss
    
    ACPO Key Feature:
    - Per-agent Value Heads V^(i) enable proper credit assignment
    - VLA forward is called only ONCE, hidden states are reused
    """
    ...
    def __init__(
        ...
        num_agents: int = 2,  # ACPO: per-agent value heads
    ):
        """
        ...
        Args:
            ...
            num_agents: Number of agents (ACPO: creates separate value heads)
        """
        super().__init__()
        ...
        # ACPO: Create per-agent Value Heads for proper credit assignment
        ...
        print_rank0(f"\nACPO: Created {num_agents} per-agent Value Heads:")
    ...
    def _setup_trainable_parameters(self):
        ...
        NOTE: Even when freeze_vla_backbone=True, gradient CAN flow through the frozen
        VLA backbone to proprio_projector. PyTorch only skips gradient computation for
        the frozen parameters themselves, but intermediate tensor gradients still propagate.
        """
        ...
        # NOTE: Gradient CAN flow through frozen VLA backbone to proprio_projector!
        # PyTorch only skips gradient for frozen params, not for input tensors.
        ...
        # ACPO: Freeze/unfreeze all per-agent value heads
        if self.value_heads is not None:
            ...
            print_rank0(f"ACPO: {self.num_agents} Value heads {'trainable' if self.train_value_head else 'frozen'}")
    ...
    def _print_trainable_summary(self):
        ...
        print_rank0("\n" + "-"*50)
        print_rank0("ACPO Trainable Parameters Summary:")
        print_rank0("-"*50)
    ...
    def get_trainable_parameters(self) -> List[nn.Parameter]:
        ...
        Returns:
            List of trainable parameters (action head + per-agent value heads + log_std)
        """
        params = []
        ...
        # ACPO: Per-agent value heads (if trainable)
    ...
    def get_critic_parameters(self) -> List[nn.Parameter]:
        """Get critic (value function) parameters - all per-agent value heads."""
        params = []
    ...
    def get_value(
        ...
        use_proprio: bool = True,
        use_film: bool = False,
        agent_idx: int = 0,  # ACPO: Use per-agent value head
    ) -> torch.Tensor:
        """
        Get state value from per-agent Value Head (ACPO).
        ...
        Args:
            ...
            use_proprio: Whether to use proprio
            use_film: Whether to use FiLM
            agent_idx: ACPO - Index of agent to use corresponding value head
        ...
        # ACPO: Use per-agent Value Head to compute value
    ...
    def get_action_and_value(
        ...
        ACPO: Uses per-agent value head V^(agent_idx).
    ...
    def evaluate_actions_and_value(
        ...
        ACPO: Uses per-agent value head V^(agent_idx) for proper credit assignment.
        ...
        # === ACPO: Per-agent value computation ===
        value = self.value_heads[agent_idx](text_hidden_states)
...
class MultiAgentVLAPolicy(nn.Module):
    """
    Multi-Agent VLA Policy for MAPPO with ACPO Advantage Decomposition.
    
    Manages multiple VLA agents with optional parameter sharing.
    
    ACPO Key Feature:
    - Per-agent value heads V^(i) are stored INSIDE VLAAgent.value_heads
    - This enables efficient computation with only ONE VLA forward pass
    - Shared VLA backbone, separate value heads for credit assignment
    ...
    def __init__(
        ...
        print_rank0("\n" + "="*60)
        print_rank0("Initializing Multi-Agent VLA Policy for MAPPO with ACPO")
        print_rank0("="*60)
        ...
        print_rank0(f"  ACPO: Per-agent Value Heads inside VLAAgent: True")
        print_rank0("="*60 + "\n")
        
        if self.share_policy:
            # ACPO: Create single shared agent with per-agent value heads INSIDE
            ...
            self.shared_agent = VLAAgent(
                ...
                num_agents=self.num_agents,  # ACPO: Creates per-agent value heads
            )
    ...
    def get_values(
        ...
        """
        Get state values for all agents using per-agent Value Heads (ACPO).
        
        ACPO: Each agent uses its own Value Head V^(i) for proper credit assignment.
        ...
        for agent_idx, agent in enumerate(self.agents[:self.num_agents]):
            ...
            # ACPO: Use per-agent value head from VLAAgent.value_heads
    ...
    def get_actions_and_values(
        ...
        ACPO: Uses per-agent Value Heads V^(i) for proper credit assignment.
        ...
        for agent_idx, agent in enumerate(self.agents[:self.num_agents]):
            ...
            # === ACPO: Per-agent value computation from shared hidden states ===
            value = agent.value_heads[agent_idx](text_hidden_states)
    ...
    def evaluate_actions_and_values(
        ...
        for agent_idx, agent in enumerate(self.agents[:self.num_agents]):
            ...
            # ACPO: Use per-agent value head with agent_idx
            ...
            log_prob, entropy, value = agent.evaluate_actions_and_value(
                ...
                use_proprio=self.cfg.use_proprio,
                use_film=self.cfg.use_film,
                agent_idx=agent_idx,  # ACPO: Use per-agent value head
            )
    ...
    def get_trainable_parameters(self) -> List[nn.Parameter]:
        ...
        - Per-agent Value head parameters (ACPO) - now inside VLAAgent
        ...
        if self.share_policy:
            # ACPO: VLAAgent.get_trainable_parameters() now includes per-agent value heads
    ...
    def forward_evaluate_agent(
        ...
        ACPO: Uses per-agent Value Head V^(agent_idx) for value computation.
