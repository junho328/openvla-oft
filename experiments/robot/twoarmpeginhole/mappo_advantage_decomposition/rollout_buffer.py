"""
Multi-Agent Rollout Buffer for MAPPO training with ACPO Advantage Decomposition.

Implements Agent-Chained Policy Optimization (ACPO) advantage computation:
1. Per-agent value storage V^(i) instead of single centralized value
2. Microstep-based GAE computation following ACPO formulation

ACPO TD Residuals:
    For agent i < N (intermediate agents):
...
class RolloutBufferSamples:
    ...
    # Per-agent values and advantages (for ACPO)
...
class MultiAgentRolloutBuffer:
    """
    Buffer for storing multi-agent rollout experience with ACPO support.
    
    Stores observations, actions, rewards, and per-agent values,
    computes ACPO-style advantages using microstep GAE, and provides minibatch sampling.
    
    ACPO Advantage Computation:
    - For intermediate agents (i < N): TD residual uses next agent's value at same timestep
    - For last agent (N): TD residual includes reward and connects to first agent at next timestep
    - Advantage computed using chain-structured GAE across agents and timesteps
    """
    ...
    def __init__(
        ...
        # Discount factors
        self.gamma = gamma
        self.gamma_prime = gamma_prime  # Microstep discount for ACPO
        self.gae_lambda = gae_lambda
        self.lambda_prime = lambda_prime  # Microstep GAE lambda for ACPO
    ...
    def _init_storage(self):
        ...
        # Per-agent value storage (for ACPO)
    ...
    def add(
        ...
        Args:
            ...
            values: List of values per agent V^(i) (for ACPO)
            reward: Team reward
    ...
    def compute_returns_and_advantages(
        ...
        """
        Compute per-agent returns and advantages using ACPO formulation.
        
        ACPO TD Residuals:
            For agent i < N (1-indexed in paper, 0-indexed here):
                ζ_t^(i) = γ' V^(i+1)([s_t, b_t^(i+1)]) - V^(i)([s_t, b_t^(i)])
                Note: Intermediate agents have R = 0
            
            For agent N-1 (last agent, 0-indexed):
                ζ_t^(N-1) = r_t + γ' V^(0)(s_{t+1}) - V^(N-1)([s_t, b_t^(N-1)])
        
        ACPO Advantage:
            A_t^(i) = Σ_{j=i}^{N-1} (γ'λ')^{j-i} ζ_t^(j) 
                    + Σ_{k=1}^∞ Σ_{j=0}^{N-1} (γ'λ')^{kN+j-i} ζ_{t+k}^(j)
        ...
        for step in reversed(range(self.buffer_size)):
            ...
            # Following ACPO formulation exactly
            td_residuals = [None] * N
            
            for i in reversed(range(N)):
                if i == N - 1:
                    # Last agent (i = N-1 in 0-indexed):
                    # ζ_t^(N) = r_t + γ' V^(1)(s_{t+1}) - V^(N)([s_t, b_t^(N)])
                    # In 0-indexed: ζ_t^(N-1) = r_t + γ' V^(0)(s_{t+1}) - V^(N-1)
                    td_residuals[i] = (
                        self.rewards[step] + 
                    ...
                    # Intermediate agents (i < N-1):
                    # ζ_t^(i) = γ' V^(i+1)([s_t, b_t^(i+1)]) - V^(i)([s_t, b_t^(i)])
                    # Note: R = 0 for intermediate agents (only last agent gets reward)
                    # V^(i+1) is at the SAME timestep, not next timestep
                    td_residuals[i] = (
                        gamma_prime * self.values[i + 1][step] - 
                        self.values[i][step]
                    )
            
            # Compute GAE advantages per agent using ACPO chain structure
            # A_t^(i) = Σ_{j=i}^{N-1} (γ'λ')^{j-i} ζ_t^(j) + (γ'λ')^{N-i} * A_{t+1}^(0) * (1 - done)
            # We process agents in reverse order to build the chain
            for i in reversed(range(N)):
                if i == N - 1:
                    # Last agent: advantage chains to first agent at next timestep
                    # A_t^(N-1) = ζ_t^(N-1) + (γ'λ') * A_{t+1}^(0) * (1 - done)
                    if step == self.buffer_size - 1:
